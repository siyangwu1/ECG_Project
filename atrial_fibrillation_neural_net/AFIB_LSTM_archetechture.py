...
def create_model(input_shape, lstm_units, dropout_rate, optimizer, loss, metrics):
    ...
    #x = Dense(units=100, kernel_initializer = tf.keras.initializers.RandomNormal(mean = 0, stddev=1), kernel_regularizer=tf.keras.regularizers.l2(), activation='tanh')(x)

    outputs = Dense(units=1, activation='sigmoid',  kernel_initializer = tf.keras.initializers.RandomNormal(mean = 0, stddev=1))(x)
